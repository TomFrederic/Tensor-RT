...
"""
    ...
    The conversion was tested using Tensorflow 1.5
"""

def chpt_to_dict_arrays_simple(file_name):
    """
        Convert checkpoint from f-lm training simulation into into a dictionary of numpy arrays 
...
def chpt_to_dict_arrays():
    ...
    print (hparams)
    # hparams = create_or_load_hparams(\
    #     FLAGS.out_dir, default_hparams, FLAGS.hparams_path, save_hparams=(FLAGS.jobid == 0))
...
def convert_rnn_kernel(weights, dimensions, is_decoder_rnn = False):
    ...
    if False :
        ...
        for i in range(layers):
            ...
            #print(weights[offset:(offset + input_size + num_units), :].shape)
            ...
            #print("layer_weights_w after reshape: {0}".format(layer_weights_w.shape))
            #print("layer_weights_r after reshape: {0}".format(layer_weights_r.shape))
            ...
            #print("Shape w", layer_weights_w.shape)
            #print("Shape r", layer_weights_r.shape)
...
def convert_weigts(dimensions, data, forget_bias = 1.0):
    ...
    # """
    # data["rnnweightp"] = np.reshape(data["rnnweightp"], (layers, cell_size, projected_size))
    # data["rnnweightp"] = np.moveaxis(data["rnnweightp"], (1,2), (2,1))
